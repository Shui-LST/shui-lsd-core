...
import {PoolContext} from "./PoolContext.sol";
...
contract PoSPool is Ownable, PoolContext, Initializable {
    ...
    uint256 public _poolLockPeriod;
    ...
    uint256 public _poolUnlockPeriod;

    string public constant VERSION = "1.3.0";

    ParamsControl public paramsControl;

    address public scfxBridge;
    ...
    modifier onlyScfxBridge() {
        require(msg.sender == scfxBridge, "Only scfxBridge can call this function");
        _;
    }
    ...
    function _updateUserInterest(address _user) private {
        ...
        uint256 latestInterest =
            accRewardPerCfx.sub(uShot.accRewardPerCfx).mul(uShot.available.mul(CFX_COUNT_OF_ONE_VOTE));
    ...
    function initialize() public initializer {
        ...
        super.initialize(0x0888000000000000000000000000000000000002,0x0888000000000000000000000000000000000005);
        RATIO_BASE = 10000;
    ...
    ///
    function increaseStake(uint64 votePower) public payable virtual onlyRegisted {
    ...
    function decreaseStake(uint64 votePower) public virtual onlyRegisted {
        require(votePower > 0, "input should be greater than zero");
        userSummaries[msg.sender].locked += userInqueues[msg.sender].collectEndedVotes();
    ...
    function withdrawStake(uint64 votePower) public onlyRegisted {
        ...
        require(_selfBalance() >= votePower * CFX_VALUE_OF_ONE_VOTE,"Not enough balance to withdraw");
    ...
    function userInterest(address _address) public view returns (uint256) {
        ...
        if (uShot.available > 0) {
            uint256 _latestInterest =
                _latestAccRewardPerCfx.sub(uShot.accRewardPerCfx).mul(uShot.available.mul(CFX_COUNT_OF_ONE_VOTE));
    ...
    function poolAPY() public view returns (uint256) {
        ...
        if (apyNodes.start != apyNodes.end) {
            for (uint256 i = apyNodes.start; i < apyNodes.end; i++) {
                ...
                totalWorkload =
                    totalWorkload.add(node.available.mul(CFX_VALUE_OF_ONE_VOTE).mul(node.endBlock - node.startBlock));
            }
        }
        ...
        if (_blockNumber() > lastPoolShot.blockNumber) {
            ...
            totalWorkload = totalWorkload.add(
                lastPoolShot.available.mul(CFX_VALUE_OF_ONE_VOTE).mul(_blockNumber() - lastPoolShot.blockNumber)
            );
        }

        if (totalReward==0 || totalWorkload==0) {
            return 0;
        }
    ...
    function userInQueue(address account, uint64 offset, uint64 limit)
        public
        view
        returns (VotePowerQueue.QueueNode[] memory)
    {
        return userInqueues[account].queueItems(offset, limit);
    }

    function userOutQueue(address account, uint64 offset, uint64 limit)
        public
        view
        returns (VotePowerQueue.QueueNode[] memory)
    {
    ...
    function lockForVotePower(uint256 amount, uint256 unlockBlockNumber) public onlyScfxBridge {
        _stakingVoteLock(amount, unlockBlockNumber);
    }

    function castVote(uint64 vote_round, ParamsControl.Vote[] calldata vote_data) public onlyScfxBridge {
    ...
    function setScfxBrdige(address _scfxBridge) public onlyOwner {
        scfxBridge = _scfxBridge;
    }
}
